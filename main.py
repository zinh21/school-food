...
if today_df.empty:
    ...
    if not today_allergy_numbers:
        ...
        food_names = [
            ALLERGY_INFO.get(num, "알 수 없음")
            for num in today_allergy_numbers
        ]

        food_list_text = ", ".join(food_names)

        st.warning(
            f"📢 **오늘 급식에는 {food_list_text}(이)가 포함되어 있어요.**\n\n"
            "**해당 알레르기가 있으신 분은 섭취에 주의해주세요!** 🙏"
        )
